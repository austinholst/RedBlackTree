...
void removal(Node* n, int side) {
  ...
  if(side != 0) {
    ...
    if(isBlack(sibling(n)->right) == true && isBlack(sibling(n)->left) == true) {
      //if n is black
      if(isBlack(n) == true) {
    ...
    if(side == 2 && (isBlack(sibling(n)->right) == true) && (isBlack(sibling(n)->left) == false)) {
      sibling(n)->right->color = BLACK;
      sibling(n)->color = RED;
      rotate_left(head, sibling(n));
    }
    else if(side == 1 && (isBlack(sibling(n)->left) == true) && (isBlack(sibling(n)->right) == false)) {
